# Athlete searches in Google Trends (2004 - Present)
...
df_athlete <- read_csv("sub_pro_15_gtrends_kenya/datasets/test_datasets/gtrends_kenyan_athletes_apr_2026.csv")
...
str(df_athlete_tidy$time)



ggplot(df_athlete_tidy, aes(x = time, y = value, color = category)) +
...
ggplot(df_athlete_tidy, aes(x = time, y = value)) +
  geom_line() +
