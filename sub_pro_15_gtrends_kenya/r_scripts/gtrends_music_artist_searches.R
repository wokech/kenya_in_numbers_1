# Music artist searches in Google Trends (2004 - Present)
...
df_artist <- read_csv("sub_pro_15_gtrends_kenya/datasets/test_datasets/gtrends_kenyan_music_artists_apr_2026.csv")

df_artist_tidy <- df_artist %>%
  pivot_longer(
...
str(df_artist_tidy$time)



ggplot(df_artist_tidy, aes(x = time, y = value, color = category)) +
...
ggplot(df_artist_tidy, aes(x = time, y = value)) +
  geom_line() +
