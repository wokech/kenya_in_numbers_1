# Kenyan TV searches in Google Trends (2004 - Present)
...
df_kenyan_tv <- read_csv("sub_pro_15_gtrends_kenya/datasets/test_datasets/gtrends_kenyan_tv_search_apr_2026.csv")

df_kenyan_tv_tidy <- df_kenyan_tv %>%
  pivot_longer(
...
str(df_kenyan_tv_tidy$time)

ggplot(df_kenyan_tv_tidy, aes(x = time, y = value, color = category)) +
...
ggplot(df_kenyan_tv_tidy, aes(x = time, y = value)) +
  geom_line() +
