# Acronym searches in Google Trends (2004 - Present)
...
df_acronym <- read_csv("sub_pro_15_gtrends_kenya/datasets/test_datasets/gtrends_kenyan_acronym_searches_apr_2026.csv")

df_acronym_tidy <- df_acronym %>%
  pivot_longer(
...
str(df_acronym_tidy$time)



ggplot(df_acronym_tidy, aes(x = time, y = value, color = category)) +
...
ggplot(df_acronym_tidy, aes(x = time, y = value)) +
  geom_line() +
